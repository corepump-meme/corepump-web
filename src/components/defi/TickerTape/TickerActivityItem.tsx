    ...
    <Link href={`/token/${token.id}`} className="flex-shrink-0">
      ...
      >
        ...
        <div className="flex-shrink-0 relative">
          {token.image ? (
            <Image
              ...
              width={20}
              height={20}
              className="w-5 h-5 rounded-full object-cover ring-1 ring-gray-300 dark:ring-white/20"
              onError={(e) => {
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2">
          ...
          {/* Amount */}
          {coreAmount && (
            <span className="text-[10px] text-gray-700 dark:text-gray-300 ml-auto">
              <span className={styles.textColor}>{formatAmount(coreAmount)}</span>
              <span className="text-gray-600 dark:text-gray-400 ml-0.5">CORE</span>
            </span>
          )}
        </div>
